...
vector<int> get_restrictions(vector<vector<int>> Board) {
    ...
    highest_row = min(highest_row, 13);
    highest_col = min(highest_col, 13);
...
//    for (int i = cor_row - 4; i <= cor_row + 4; i++) {
//        if (i >= 0 && i < num_rows){
//            vertical_direction.push_back(Board[i][cor_col]);
//        }
//    }
//
//    for (int j = cor_col - 4; j <= cor_col + 4; j++) {
//        if (j >= 0 && j < num_columns){
//            horizontal_direction.push_back(Board[cor_row][j]);
//        }
//    }
//
//    for (int i = cor_row - 4, j = cor_col - 4; i <= cor_row + 4 and j <= cor_col + 4; i++, j++) {
//        if (i >= 0 && i < num_rows && j >= 0 && j < num_columns) {
//            leftdiag_direction.push_back(Board[i][j]);
//        }
//    }
//    for (int i = cor_row - 4, j = cor_col + 4; i <= cor_row + 4 and j >= cor_col - 4; i++, j--) {
//        if (i >= 0 && i < 4 && j >= 0 && j < 4){
//            rightdiag_direction.push_back(Board[i][j]);
//        }
//    }
    for(int i = -4; i < 5; i++){
...
bool time_out() {
    ...
    auto time_interval = chrono::duration_cast<chrono::milliseconds>(finish - start);
    if (time_interval.count() >= MaximumTime)
        return true;
    else
        return false;
}
...
Move iterative_negamax(int player, vector<vector<int>> Board, int time = 60000) {
    Table_init();
    ::start = chrono::high_resolution_clock::now();
    MaximumTime = time;
    Move best_move(6, 6, 0);
    int depth = 4;
    MaximumDepth = depth;
    while (time_out() == false) {
        int temp_bestscore;
        temp_bestscore = negamax(Board, player, MaximumDepth, -inf, inf, Hash(Board), get_restrictions(Board), -1, -1);
//        best_move = Best_Move;
        if (time_out() == true) {
            break;
        }
        best_move = Best_Move;
        if (best_move.score > 1999900) {
            break;
        }
        depth += 2;
    }
...
Move iterative_negascout(int player, vector<vector<int>> Board, int time = 60000) {
    Table_init();
    ::start = chrono::high_resolution_clock::now();
    MaximumTime = time;
    Move best_move(7, 7, 0);
    int depth = 4;
    MaximumDepth = depth;
    while (time_out() == false) {
        int temp_bestscore;
...
int main() {
        ...
        if (crt == 'x') {
            ...
            while (terminal(GameBoard) == false) {
                ...
                GameBoard[r][c] = 1;
                PrintBoard(GameBoard);
                Move best_move = iterative_negascout(-1, GameBoard, 10000);
            ...
            GameBoard[ro][co] = -1;
            while (terminal(GameBoard) == false) {
                Move best_move = iterative_negascout(1, GameBoard, 2000);
